...
\begin{itemize}
\item \mtex{diff}: diff operator, $\diff\ $, $\int_0^t f(\tau) \diff \tau$
\item \mtex{Diff}: Diff operator, $\Diff\ $, $\Diff{}^2 X = \dfrac{-x_{i + 1, j} + 2x_{i, j} - x_{i - 1, j}}{\Delta x^2}$
\item \mtex{Expect}: Expect operator, $\Expect$, $X = B(n, p)$, $\Expect X = np$
\item \mtex{diag}, \mtex{eig}, \mtex{tr}: Matrix operators, $\diag$, $\eig$, $\tr$, $\mmd = \diag \mma$, $[\mmlambda, \mmv] = \eig \mma$, $\tr \mmlambda = \tr \mma$
\item \mtex{lcm}: lcm operator, $\lcm$, $\lcm(f, g) \cdot \gcd(f, g) = f \cdot g$
\item \mtex{rand}: random number, $\rand$
\item \mtex{mean}, \mtex{var}: statistics operator, $\mean$, $\var$, $\mu = \mean X$, $\sigma^2 = \var X$
\item \mtex{corr}: correlation operator, $\corr$, $\corr(X, Y) = (R)_{ij} = \dfrac{\sum_{X_i, Y_j}(X - \bar X)(Y - \bar Y)}{\sqrt{\sum_i(X - \bar X)^2\sum_j(Y - \bar Y)^2}}$
\item \mtex{conv}: convolution operator, $\conv$, $\conv(f, g) = \int_{-\infty}^\infty f(\tau)g(t - \tau) \diff \tau$
\item \mtex{card}: cardinals operator, $\card$, $\card \{1, 2, 3\} = 3$, $\card \mreal = 2^{\aleph_0}$
\item \mtex{argmin}, \mtex{argmax}, \mtex{argopt}: $\argmin$, $\argmax$, $\argopt$ operator, $\hat \theta = \argmin\limits_\theta J_\theta(x)$
\item \mtex{dist}: distance operator, $\dist$, $\min \sum\limits_{\forall s, t \in G} \dist\limits_{s \ne t} (s, t)$
